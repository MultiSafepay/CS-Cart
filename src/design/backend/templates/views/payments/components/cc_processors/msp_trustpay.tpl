...
{assign var="r_url" value="`$config.http_location`/`$config.customer_index`?dispatch=payment_notification.notify&payment=multisafepay_trustpay"}
{assign var="e_url" value="`$config.http_location`/`$config.customer_index`?dispatch=payment_notification&payment_notification.result=multisafepay_trustpay"}
<h3>MultiSafepay</h3>
<p />
...
<input type="hidden" name="payment_data[processor_params][gateway]" maxlength="20" id="msp_gateway" value="TRUSTPAY" class="input-text" />

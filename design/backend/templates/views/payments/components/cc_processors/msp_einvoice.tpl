...
{assign var="r_url" value="`$config.http_location`/`$config.customer_index`?dispatch=payment_notification.notify&payment=multisafepay_einvoice"}
{assign var="e_url" value="`$config.http_location`/`$config.customer_index`?dispatch=payment_notification&payment_notification.result=multisafepay_einvoice"}
<h3>MultiSafepay</h3>
<p />
...
<input type="hidden" name="payment_data[processor_params][gateway]" maxlength="20" id="store_id" value="EINVOICE" class="input-text" />
...
<div class="form-field">
  ...
  <select name="payment_data[processor_params][debug]" id="debug">
    <option value="NO" {if $processor_params.debug == "NO"}selected="selected"{/if}>NO</option>
    <option value="YES" {if $processor_params.debug == "YES"}selected="selected"{/if}>YES</option>
  </select>
</div>
